...
template<>
void BoussinesqForce<FlowOverWaveSolver>::add_forces(
  ...
  const Kokkos::DefaultExecutionSpace& space) const
{
  ...
  if (theta0 == std::numeric_limits<double>::max()) {
    add_buoyancy_force_without_ref_scalar_curve(
      fu.z, scalar->array, Ri, solver.flow_field.mesh, space);
  } else {
    add_buoyancy_force_with_ref_scalar_curve(
...
namespace alps::solver {
template class BoussinesqForce<FlowOverWaveSolver>;
} // namespace alps::solver
