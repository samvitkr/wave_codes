...
constexpr auto app_name = "alps_obl";
...
void integrate_solver(solver::ChannelFlowSolverAB2& solver,
                      ...
                      ConfigTable const&            config)
{
  ...
  while (!progress_tracker.is_complete()) {
    ...
    solver.project();
    solver.correct();
    apply_translation_to_fix_bottom_mean_u(solver.flow_field,
             ...
             alps::CLIOptions const& cli_options)
{
  ...
  solver::ChannelFlowSolverAB2 ns_solver(flow, solver_options);

  {
    auto const Us_config = config.extract_table("VortexForce.StokesDrift");
    auto       Us_model  = create_stokes_drift_from_config(Us_config);
    ns_solver.body_forces.emplace_back(
      std::make_unique<VortexForce>(ns_solver, Us_model));
  }
