...
%phinlm=pscm;
%%
fphi=fullfile(baseDir,'spectra.mat');
 save(fphi,'phinl','phiad','phist','kx','ky','zz');

%%

% Now test Parseval's Theorem using the integrals
Tnlphi_integral = squeeze(real(sum(phinl, [1 2]))) * dkx * dky;
% % Tnla_integral   = squeeze(sum(nla, [1 2])) * dX * dY;
% % % Tnlphi_integral and Tnla_integral will now match perfectly
% % Tnla =squeeze( sum(nla,[1 2]));
% % Tnlphi= squeeze(real(sum(phinl,[1 2])));
% % 
% % %%
% % figure
% % hold on
% % plot(Tnla_integral,zz,'o')
% % plot(Tnlphi_integral,zz,'-r')
% % hold off
