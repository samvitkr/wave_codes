...
void FreeSurfaceSolver::advance_scalars_ab2()
{
  ...
  for (std::size_t i_scalar = 0; i_scalar < flow_field.scalars.size();
    ...
    invoke_advection_fluxes_functor<CurveAdvectionFunctor<TopWaveMesh>>(
...
void FreeSurfaceSolver::scale_scalars_by_J() const
{
  auto const& mesh = flow_field.mesh;
  auto const& J    = mesh.J;
