...
% % Fu_2D_phase = griddedInterpolant(XI_2D_phase, ZETA_2D_phase, zeros(size(XI_2D_phase)), 'linear', 'linear');
% % % --------------------------------------------------------------------------
% % 
% % load(fullfile(baseDir,'mean_phase_fields.mat'),'viscpav','strpav','convpav');
% % 
% % % Use the new phase-averaged interpolant instead of the full-grid one
% % Fu_2D_phase.Values = viscpav ./ up(1:Nxl,:);
% % visc_sl = Fu_2D_phase(XQ, ZETA_Q);
% % 
% % Fu_2D_phase.Values = strpav ./ up(1:Nxl,:);
% % str_sl = Fu_2D_phase(XQ, ZETA_Q);
% % 
% % Fu_2D_phase.Values = convpav ./ up(1:Nxl,:);
% % conv_sl = Fu_2D_phase(XQ, ZETA_Q);
% 
% 
% convm=zeros(numl,1);
% viscm = zeros(numl,1);
...
% 
% 	convm(k) = trapz(XQ(:,k),conv_sl(:,k));
%         strm(k) = trapz(XQ(:,k),str_sl(:,k));
%         viscm(k) = trapz(XQ(:,k),visc_sl(:,k));
% end
% 
% %%
% 
