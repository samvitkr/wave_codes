...
void FreeSurfaceSolver::save(std::filesystem::path grid_file,
                             ...
                             std::filesystem::path aux_file) const
{
...

  {
    ...
    // save top pressure, top processors only
    auto              is_top = mesh.comm().is_last(2);
    ...
    const auto offset_xy =
      is_top ? std::vector{partition.offsets[0], partition.offsets[1]}
             : std::vector{0, 0};
    io::hdf5::write_blocks(aux_data,
                           "pf0",
                           flow_field.top_bc->p_top_0,
                           total_shape_xy,
                           block_shape_xy,
                           offset_xy);
  }
...
void FreeSurfaceSolver::load(std::filesystem::path grid_file,
                             std::filesystem::path data_file,
                             std::filesystem::path aux_file)
{
  ...
  if (std::filesystem::exists(data_file)) {
    ...
    if (h5file.exist("eta")) {
      ...
      if (mesh.comm().rank() == 0) {
        logger->warn("'eta' are not found in the data file; they "
                     "are set to 0.");
      }
      Kokkos::deep_copy(flow_field.eta, 0);
      Kokkos::deep_copy(flow_field.mesh.et, 0);
    }
  ...
  if (std::filesystem::exists(aux_file)) {
    ...
    if (aux_data.exist("/hu") && aux_data.exist("/hv") && aux_data.exist("/hw")
        && aux_data.exist("/pf0")) {
      ...
      io::hdf5::read3D_xyz(aux_data, mesh.partition(), "hu", Ru_saved.x.view());
      io::hdf5::read3D_xyz(aux_data, mesh.partition(), "hv", Ru_saved.y.view());
      io::hdf5::read3D_xyz(aux_data, mesh.partition(), "hw", Ru_saved.z.view());

      // read top pressure on top processors only
      auto              is_top = mesh.comm().is_last(2);
      const std::vector total_shape_xy{mesh.global_extent(0),
                                       mesh.global_extent(1)};

      const auto block_shape_xy = is_top
                                  ? std::vector{mesh.extent(0), mesh.extent(1)}
                                  : std::vector{0, 0};
      const auto offset_xy = is_top ? std::vector{mesh.partition().offsets[0],
                                                  mesh.partition().offsets[1]}
                                    : std::vector{0, 0};
      io::hdf5::read_blocks(aux_data,
                            "pf0",
                            flow_field.top_bc->p_top_0,
                            total_shape_xy,
                            block_shape_xy,
      ...
      if (mesh.comm().rank() == 0) {
        logger->warn("hu, hv, hw or pf0 not found in restart data");
      }
    }
