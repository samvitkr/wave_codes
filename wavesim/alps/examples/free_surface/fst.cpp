...
constexpr auto app_name = "alps_fst";
...
void integrate_solver(solver::FreeSurfaceSolver& solver,
                      ...
                      ConfigTable const&         config)
{
  ...
  // Create a temporary flat field to pass into the statistics
  auto const flow_field_flat = solver.flow_field.as_channel_flow_field();
  statistical_output.attach_statistics(diagnostics::create_velocity_statistics(
    ...
    std::unordered_set{diagnostics::FieldStatisticsType::XY,
                       diagnostics::FieldStatisticsType::Y}));
  ...
  while (!progress_tracker.is_complete()) {
    ...
    {
      // Update eta_t, which is needed for the rhs calculation
      auto const& velocity = solver.flow_field.u;
      auto const& mesh     = solver.flow_field.mesh;
      auto        nz       = mesh.extent(2);
      solver.flow_field.top_bc->update_eta_t(
        mesh,
        subview(velocity.x, ALL, ALL, nz - 1).view(),
        subview(velocity.y, ALL, ALL, nz - 1).view(),
        subview(velocity.z, ALL, ALL, nz - 2).view());
    }

    auto Ru = solver.calc_explicit_rhs();
    solver.set_time(new_time); // time needs to be set so in calc_uhat the
                               // updated BCs are correctly calculated
    solver.calc_uhat(Ru, 1);

    solver.project(1);
    solver.correct(1);

    solver.calc_uhat(std::move(Ru), 2);
...
void program(ConfigTable const& root_config, CLIOptions const& cli_options)
{
  ...
  auto solver_options = solver::FreeSurfaceSolverOptions::parse_from(config);
  ...
  solver::TopWaveMesh mesh(grid, (Real)domain_size[2]);

  solver::FreeSurfaceFlowField flow(mesh);
  flow.parse_bcs_from(config);
  flow.initialize_scalars(config);

  solver::FreeSurfaceSolver ns_solver(flow, solver_options);
  ...
  if (comm.rank() == 0) spdlog::info("{}", fmt::streamed(ns_solver));

  ns_solver.initialize();
  Kokkos::fence();

  integrate_solver(ns_solver, save_options, config);
  Kokkos::fence();
