...
constexpr auto app_name = "alps_wind_wave";
...
void integrate_solver(solver::FlowOverWaveSolver& solver,
             ...
             alps::CLIOptions const& cli_options)
{
  ...
  auto solver_options = solver::FlowOverWaveSolverOptions::parse_from(config);
  ...
  solver::BottomWaveMesh mesh(grid, (Real)domain_size[2]);

  solver::FlowOverWaveField flow(mesh);
  flow.parse_bcs_from(config);
  flow.initialize_scalars(config);

  solver::FlowOverWaveSolver ns_solver(flow, solver_options);
