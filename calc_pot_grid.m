...
if any(nan_idx)
    ...
    F = scatteredInterpolant(nodes(1,:)', nodes(2,:)', sol, 'linear', 'boundary');
...
Nan_2d = single(reshape(nan_idx, [Nx, Nz])); 
fprintf('Saving 2D interpolated potential to %s...\n', fn_out);
fprintf('Interpolation complete.\n');

% % Evaluate analytical gradients directly from the PDE solution
% [gradx, gradz] = evaluateGradient(phiexact, xq, zq);
% % Apply scatteredInterpolant to handle NaNs in gradx and gradz using 'linear' extrapolation
% % Reshape directly into velocity fields
% uphi = single(reshape(gradx, [Nx, Nz]));
% wphi = single(reshape(gradz, [Nx, Nz]));
% 
% 
% % Evaluate analytical gradients on your regular 2D query grid
[gradx, gradz] = evaluateGradient(phiexact, xq, zq);

% Extract the original unstructured nodes
nodes = phiexact.Mesh.Nodes;

% Evaluate gradients directly at the original PDE mesh nodes to use as interpolation source data
[gradx_nodes, gradz_nodes] = evaluateGradient(phiexact);

% Fix NaNs in gradx (uphi)
nan_idx_u = isnan(gradx);
if any(nan_idx_u)
    fprintf('Found %d NaN points in uphi. Fixing with nearest-neighbor extrapolation...\n', sum(nan_idx_u));
    % Note: Changed 'boundary' to 'nearest' as MATLAB expects 'nearest' for extrapolation
    Fu = scatteredInterpolant(nodes(1,:)', nodes(2,:)', gradx_nodes(:), 'linear', 'linear');
    gradx(nan_idx_u) = Fu(xq(nan_idx_u)', zq(nan_idx_u)');
end

% Fix NaNs in gradz (wphi)
nan_idx_w = isnan(gradz);
if any(nan_idx_w)
    fprintf('Found %d NaN points in wphi. Fixing with nearest-neighbor extrapolation...\n', sum(nan_idx_w));
    Fw = scatteredInterpolant(nodes(1,:)', nodes(2,:)', gradz_nodes(:), 'linear', 'linear');
    gradz(nan_idx_w) = Fw(xq(nan_idx_w)', zq(nan_idx_w)');
end

% Reshape cleanly into the final 2D velocity fields
uphi = single(reshape(gradx, [Nx, Nz]));
wphi = single(reshape(gradz, [Nx, Nz]));
%%
fprintf('Gradient interpolation and extrapolation complete.\n');
zz   = h5read(fname,'/zz');
% dzw =diff(zw)';
