...
{
 public:
  explicit FreeSurfaceFlowField(const TopWaveMesh& local_mesh);
  ...
  void set_top_bc(std::unique_ptr<FreeSurfaceBC> bc) const;
  ...
  mutable std::unique_ptr<FreeSurfaceBC> top_bc;
...
template<>
struct is_curvilinear_field<FreeSurfaceFlowField> : std::true_type
{};
