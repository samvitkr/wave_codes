...
void invoke_convection_fluxes_functor(const Tensor33Field<Real***>& fluxes,
                                      const FieldType&              flow)
{
  Kokkos::Profiling::pushRegion("convection");
  std::vector<Kokkos::DefaultExecutionSpace> streams;

  auto       begins  = local_begins(flow.u.x);
  auto       ends    = local_ends(flow.u.x);
  const auto functor = Functor(fluxes, flow.mesh, flow.u);
  if (flow.mesh.comm().is_first(2)) {
    ...
    LoopPolicy<2, typename Functor::Bottom_0> policy_bottom_0(
      ...
      {ends[0], ends[1]},
      {16, 8});
    Kokkos::parallel_for("convection_bottom_0", policy_bottom_0, functor);
  }
  if (flow.mesh.comm().is_last(2)) {
    ...
    LoopPolicy<2, typename Functor::Top_NzMinus2> policy_top_2(
      ...
      {ends[0], ends[1]},
      {16, 8});
    Kokkos::parallel_for("convection_top_2", policy_top_2, functor);
    Kokkos::parallel_for("convection_top_1", policy_top_1, functor);
  }
  ...
  Kokkos::parallel_for("convection", policy, functor);
