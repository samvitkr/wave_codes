                      ...
                      ConfigTable const&      config)
{
  ...
  auto pressure_impulse_config = config.extract_table("pressure_impulse");
  auto x0_initial =
    pressure_impulse_config.get_value<std::vector<double>>("x0");

  auto constexpr PI = Kokkos::numbers::pi_v<double>;
  auto x0           = 2 * PI * x0_initial.at(0) / solver.grid.pex;
  auto y0           = 2 * PI * x0_initial.at(1) / solver.grid.pey;
  auto R            = pressure_impulse_config.get_value<double>("R");
  auto p0           = pressure_impulse_config.get_value<double>("p0");
  auto U0           = pressure_impulse_config.get_value<double>("U");

  auto pa_func = [&grid = solver.grid, x0, y0, R, p0, U0](
                   decltype(solver::hos::HOSField::pa)  pa,
                   double                               time,
                   Kokkos::DefaultExecutionSpace const& space) {
    auto pmax = p0 * (1 - std::exp(-time * time * 20));
    alps::radial_moving_pressure(pa, time, pmax, R, {x0, y0}, U0, grid, space);
  };
  ...
  while (!progress_tracker.is_complete()) {
    ...
    solver.do_step((Real)dt, pa_func);
