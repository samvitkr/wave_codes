...
void FlowOverWaveSolver::apply_scalar_bc(
  ...
  WhichBoundary                        boundary) const
{
  ...
  if (boundary == WhichBoundary::BottomBC || boundary == WhichBoundary::Both) {
    ...
    if (auto const* bc =
        ...
        bc != nullptr) {
      apply_bottom_bc(scalar.array, *bc, flow_field.mesh, space);
      bc_set = true;
    }
    if (auto const* bc =
    ...
    if (auto const* bc =
        ...
        bc != nullptr) {
      auto const scalarD = 1 / (options.Re * scalar_options.ScPr.value());
      apply_bottom_bc(scalar.array, *bc, (Real)scalarD, flow_field.mesh, space);
      bc_set = true;
    }
...
void FlowOverWaveSolver::set_boundary_scalar_flux(
