...
{
  ALPS_BENCH_LOG("n.type=" << static_cast<int>(n.type));
...
static bool matchCmdArgs(std::string const&        str,
...
class MarkDownColumn
{
 public:
  ...
  [[nodiscard]] std::string invalid() const;
                                ...
                                std::vector<std::string>& recommendations)
{
  ...
  if (nprocs <= 0) {
    warnings.emplace_back("couldn't figure out number of processors - no "
                          "governor, turbo check possible");
  } else {
    ...
    for (long id = 0; id < nprocs; ++id) {
      ...
      auto minFreq =
        parseFile<int64_t>(sysCpu + "/cpufreq/scaling_min_freq", nullptr);
      auto maxFreq =
        parseFile<int64_t>(sysCpu + "/cpufreq/scaling_max_freq", nullptr);
      if (minFreq != maxFreq) {
    ...
    auto noTurbo =
      parseFile<int>("/sys/devices/system/cpu/intel_pstate/no_turbo", &fail);
    if (!fail && noTurbo == 0) {
...
struct IterationLogic::Impl
{
    ...
    , mResult(bench.config())
  {
    ...
    if (mBench.comm_size() > 1) {
      // in MPI mode, we need to make sure that all ranks use the same number
      // of iterations. We use the maximum number of iterations across all ranks
      uint64_t iters = mNumIters;
      MPI_Allreduce(
        &iters, &mNumIters, 1, MPI_UINT64_T, MPI_MAX, mBench.comm());
    }
  }

  // directly calculates new iters based on elapsed&iters, and adds a 10% noise.
  // Makes sure we don't underflow.
  [[nodiscard]]
  uint64_t calcBestNumIters(std::chrono::nanoseconds elapsed,
                            uint64_t                 iters) noexcept
  {
    ...
    auto new_iters = static_cast<uint64_t>(doubleNewIters + 0.5);

    if (mBench.comm_size() == 1) return new_iters;

    // in MPI mode, we need to make sure that all ranks use the same number of
    // iterations. We use the maximum number of iterations across all ranks
    uint64_t itersLocal = new_iters;
    MPI_Allreduce(
      &itersLocal, &new_iters, 1, MPI_UINT64_T, MPI_MAX, mBench.comm());
    return new_iters;
  }
  ...
  {
#if defined(ALPS_BENCH_LOG_ENABLED)
  ...
  [[nodiscard]]
  bool
  isCloseEnoughForMeasurements(std::chrono::nanoseconds elapsed) const noexcept
  {
    auto local = elapsed * 3 >= mTargetRuntimePerEpoch * 2;
    if (mBench.comm_size() == 1) return local;

    // in MPI mode, we need to make sure that all ranks agree that we are
    // close enough
    int localInt = local ? 1 : 0;
    int globalInt{};
    MPI_Allreduce(&localInt, &globalInt, 1, MPI_INT, MPI_MIN, mBench.comm());
    return globalInt != 0;
  }
...
{
  ALPS_BENCH_LOG(mPimpl->mBench.name() << ": mNumIters=" << mPimpl->mNumIters);
...
class LinuxPerformanceCounters
{
 public:
  ...
  [[nodiscard]] bool hasError() const noexcept { return mHasError; }
...
bool LinuxPerformanceCounters::monitor(uint32_t type,
                                       uint64_t eventid,
                                       Target   target)
{
  ...
  , mHas()
{
  ...
  mPc->start();
  mPc->calibrate([] {
    auto before = alps::bench::Clock::now();
    auto after  = alps::bench::Clock::now();
    (void)before;
    (void)after;
  });
...
Config& Config::operator=(Config&&) noexcept(ALPS_BENCH(NOEXCEPT_STRING_MOVE)) =
  default;
...
Result& Result::operator=(Result&&) noexcept(ALPS_BENCH(NOEXCEPT_STRING_MOVE)) =
  default;
...
void Result::add(Clock::duration                    totalElapsed,
                 uint64_t                           iters,
                 detail::PerformanceCounters const& pc)
{
  ...
  if (pc.has().branchInstructions) {
    ...
    // correcting branches: remove branch introduced by the while (...) loop for
    // each iteration.
...
Bench&
Bench::operator=(Bench&&) noexcept(ALPS_BENCH(NOEXCEPT_STRING_MOVE)) = default;
...
Bench& Bench::timeUnit(std::chrono::duration<double> const& tu,
...
Bench& Bench::context(std::string const& variableName,
...
[[nodiscard]] std::ostream* Bench::output() const noexcept
{
  return mConfig.mOut;
}
...
{
  ::alps::bench::render(templateContent, *this, os);
  return *this;
}
...
Bench& Bench::comm(MPI_Comm comm) noexcept
{
  mConfig.mComm = comm;
  return *this;
}

MPI_Comm Bench::comm() const noexcept
{
  return mConfig.mComm;
}

int Bench::comm_size() const noexcept
{
  if (mConfig.mComm == MPI_COMM_NULL) {
    return 1;
  }
  int size{};
  if (int err = MPI_Comm_size(mConfig.mComm, &size); err != MPI_SUCCESS) {
    return 1;
  }
  return size;
}
  ...
  , mY(0)
{
  if (data.size() != 2) {
    throw std::runtime_error(
      "alps::bench::Rng::Rng: needed exactly 2 entries in data, but got "
