             ...
             alps::CLIOptions const& cli_options)
{
  ...
  {
    auto const radiant_heating_config = config.extract_table("RadiantHeating");
    auto       source_term            = std::make_unique<RadiantHeating>(
      RadiantHeating::parse_from_config(radiant_heating_config, ns_solver));
    ns_solver.scalar_sources.at(0).push_back(std::move(source_term));
  }
